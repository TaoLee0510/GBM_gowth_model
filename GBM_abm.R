...
# on an N×N grid.
#   • All per-hour logic (division activation, division attempts, migration, and all death logic)
#     is implemented in C++ via hourly_step_core_cpp; R contains no fallback implementations.
...
# Build per-id maps (used for initial bulk r via C++; not needed per-hour since C++ updates r)
...
run_simulation_cfg <- function(cfg, karyolib, outputdir, prefix = "Cells", global_id = NA_integer_) {
  ...
  while (step < max_steps) {
    ...
    #   - normal-division activation, risk-set marking, division (with MS), bulk r update,
    #     resource allocation & deaths (all modes), random deaths, timeout deaths, migration
